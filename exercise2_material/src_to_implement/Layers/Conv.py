...
class Conv(Base.BaseLayer):
    ...
    def forward(self, input_tensor):
       ...
       # For Even Kernel sizes:
        ...
        # dimensions of the output or kernel movements:
    ...
    def backward(self, error_tensor):
        ...
        # Gradient_bias
        if error_tensor.ndim == 3:  # 1D Convolution
           self.gradient_bias = np.sum(error_tensor, axis=(0, 2))
        elif error_tensor.ndim == 4:  # 2D Convolution
           self.gradient_bias = np.sum(error_tensor, axis=(0, 2, 3))
        else:
            raise ValueError("Unsupported error_tensor shape.")
    ...
    # Initialization :   initialize(self, weights_shape, fan_in, fan_out)
